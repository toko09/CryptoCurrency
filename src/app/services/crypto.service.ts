...
export class CryptoService {
  ...
  getSearch(searchText: string) {
    this.http
      ...
      .subscribe((result) => {
        let searchedObject = result.data.find(
          (item) =>
            item.symbol.toLocaleLowerCase() === searchText.toLocaleLowerCase()
        );

        if (!searchedObject) {
          searchedObject = result.data.find(
            (item) =>
              item.nameid.toLocaleLowerCase() === searchText.toLocaleLowerCase()
          );
        }
